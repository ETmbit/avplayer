...
basic.forever(function () {
    if (ESerial.available()) {
        ...
        if (RPIMSG == "RDY")
            ...
            if (ix >= 0) {
                RPITXT = RPIMSG.substr(ix + 1)
                RPIMSG = RPIMSG.substr(0, ix)
                ix = RPITXT.indexOf("|")
                if (ix >= 0) {
                    RPINUM = parseFloat(RPITXT.substr(ix + 1))
                    RPITXT = RPITXT.substr(0, ix)
                }
            }
            if (mediaHandler)
                mediaHandler()
...
namespace AVPlayer {
    ...
    //% color="#FFC000"
    //% subcategory="Instructies"
    //% block="on receiving a command"
    //% block.loc.nl="wanneer een instructie is ontvangen"
    ...
    //% block="start video %name"
    //% block.loc.nl="start video %name"
